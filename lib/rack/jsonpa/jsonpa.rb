...
module Rack
  ...
  class JSONPa
    ...
    #
    def format_jsonp(callback, status, headers, response)
      x_headers = headers.inject({}) {|result, (k,v)| (k =~ /^X-(.+)/i) ? result[$1] < v : result }
      response = response.inject('') {|result, r| result << r.to_s }
      #response = {status: status, headers: x_headers}.to_json[0..-2] + ",\"body\":#{response}}"
      ["#{callback}(#{response}, #{status}, #{x_headers})"]
    end
